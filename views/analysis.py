...
def load_view(comp_id):
    ...
    if analyzeBtn:
        if not selections: st.error("Error: Select a model from the sidebar first!")
        if uploaded_file:
            frames_flag = 0
         ...
         ####################### Adding the analysis to the database #################################
            ...
            st.error("ERROR: No video found, please select a video and try again!")

        ############################### Generate a detailed report ###################################
        if reportBx:
            try:
                with st.expander("More analysis details"):
                    with st.container(): # Large container than includes more containers
                        with st.container():
